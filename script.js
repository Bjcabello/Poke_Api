const apiURL = 'https://pokeapi.co/api/v2/pokemon/';

const buscarTexto = document.getElementById('buscar');
const contenedorPokedex = document.getElementById('pokedex');

// Función para mostrar un mensaje de error
function showError(message) {
    contenedorPokedex.innerHTML = `<p class="error">${message}</p>`;
}

// Función para buscar un Pokémon
async function buscarPokemon(mensaje, color="white") {
    // Obtener el valor del campo de búsqueda y convertirlo a minúsculas
    const busquedaPokemon = buscarTexto.value.toLowerCase();
    console.log(`${color}`);
    try {
        // Realizar una petición a la API de PokeAPI con el nombre del Pokémon
        const respuesta = await fetch(apiURL + busquedaPokemon);
        if (!respuesta.ok) {
            // Si la respuesta no es exitosa, mostrar un mensaje de error
            showError(`No se encontró ningún Pokémon llamado "${busquedaPokemon}"`);
            return;
        }

        // Convertir la respuesta a JSON
        const data = await respuesta.json();

        // Mostrar los datos del Pokémon en el contenedor de resultados
        contenedorPokedex.innerHTML = 
        `
            <h2>${data.name.toUpperCase()}</h2>
            <img src="${data.sprites.front_default}" alt="${data.name}">
            <p>Número: ${data.id}</p>
            <p>Altura: ${data.height / 10}m</p>
            <p>Peso: ${data.weight / 10}km</p>
        `;
    } catch (error) {
        // Si ocurre algún error durante la petición, mostrar un mensaje de error
        showError('Ha ocurrido un error al buscar el Pokémon ');
        console.error(error);
    }
}

// Agregar un controlador de eventos al botón de búsqueda
document.querySelector('button').addEventListener('click', buscarPokemon);